...
//     wave's census (~113 000 allocation-bearing entries on the host's own tree, ~363 000 on the large
...
// ── LAYOUT PINS ───────────────────────────────────────────────────────────────────────────────────────
// The host tree has already shipped a change that silently made its fixed-width string type read 16 bytes
// instead of 32, with no test to catch it. These fail the BUILD instead, and the host's layer gate mutates
// a scratch copy of this header to prove they actually fire rather than merely existing. The pins are
// spelled with the primitive rather than with the host's 32-bit id alias — the same type — so this header
// stays free of host names and stays vendorable.
//
...
// conversion wave multiplies every byte here by ~113 000 entries on the host's own tree and ~363 000 on
